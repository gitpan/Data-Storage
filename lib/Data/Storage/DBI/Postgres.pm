...
The superclass L<Data::Storage::DBI> defines these methods and functions:
    ...
    clear_AutoCommit(), clear_HandleError(), clear_LongReadLen(),
    clear_PrintError(), clear_RaiseError(), clear_dbh(), clear_dbhost(),
    clear_dbname(), clear_dbpass(), clear_dbuser(), clear_port(),
    clear_schema_prefix(), commit(), connect_string(), dbh(), dbh_clear(),
    dbhost(), dbhost_clear(), dbname(), dbname_clear(), dbpass(),
    dbpass_clear(), dbuser(), dbuser_clear(), disconnect(), except(),
    finally(), get_connect_options(), is_connected(), lazy_connect(),
    otherwise(), port(), port_clear(), prepare(), rewrite_query(),
    rollback(), schema_prefix(), schema_prefix_clear(), signature(), try(),
    with()
...
This document describes version 0.05 of L<Data::Storage::DBI::Postgres>.
